import NextLink from "next/link";
import Marquee from "react-fast-marquee";

type LinkKey =
| "characters"
| "mtwim"
| "games"
| "pp"
| "gallery"
| "blog"
| "about";

type LinkItemProps = {
  desc: string;
  active: LinkKey;
  link: LinkKey;
  onHover: (link: LinkItemProps["link"]) => void;
};

export default function LinkItem({ desc, active, link, onHover }: LinkItemProps) {
  return (
    <NextLink
      href={`/${link}`}
      className={`relative overflow-hidden rounded-t-xl ${active === link ? "bg-[#17191a]" : "bg-[#17191a]/50"}`}
      onMouseEnter={() => onHover(link)}
    >
      <p className={`h-10 flex items-center pl-3 gap-2.5 ${active === link ? "text-yellow-300 white-glow" : "text-white/50"}`}><span className={`${active === link ? "text-xl spin" : ""}`}>★</span><span className={active === link ? "underline" : ""}>{active === link ? `${link}!` : `${link}`}</span></p>
      <p className={`z-50 transition-opacity ${active === link ? "opacity-100 duration-500" : "opacity-0 duration-0"} absolute meow text-xs bottom-1 right-2`}>{desc}</p>
      <Marquee>
        <div className="-mr-px">
          <img src={`/images/${link}.png`} className="h-40 nonsel" />
        </div>
      </Marquee>
    </NextLink>
  );
}